...
export function getSecrets(
	...
	const oauthAccessToken = getNestedProperty(credentials, 'oauthTokenData.access_token');
...
export const prepareRequestBody = async (
	...
	if (bodyType === 'json' && version >= 4) {
		return await parameters.reduce(async (acc, entry) => {
			...
			setNestedProperty(result, entry.name, entry.value);
			return result;
		}, Promise.resolve({}));
...
export function toText<T>(data: T) {
